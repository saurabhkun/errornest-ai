...
import { NewIssueEmail, RegressionEmail, SpikeEmail, InviteEmail } from "./EmailTemplates";
...
export class EmailDispatcher {
  ...
  static async sendInviteEmail(params: {
    ...
    const { renderToStaticMarkup } = await import("react-dom/server");
    const baseUrl = process.env.NEXTAUTH_URL || "http://localhost:3000";
    const inviteUrl = `${baseUrl}/invites/accept?token=${params.token}`;
    const subject = `Invitation to join ${params.orgName} on ErrorNest`;
    ...
    try {
      ...
      const apiKey = process.env.RESEND_API_KEY;
      if (!apiKey || apiKey === "mock") {
        console.log(`[MOCK INVITE EMAIL] To: ${params.email}`);
        console.log(`Subject: ${subject}`);
        console.log(`HTML length: ${html.length} chars`);
        return;
      }
      ...
      const resBody = await response.json();
      console.log(
        `Invite email successfully dispatched via Resend to ${params.email}. ID: ${resBody.id}`
      );
    } catch (error) {
      console.error(`Failed to dispatch invite email to ${params.email}:`, error);
      throw error;
    }
  }
}
